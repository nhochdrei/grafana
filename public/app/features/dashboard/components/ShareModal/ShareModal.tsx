...
export class ShareModalUnthemed extends PureComponent<Props, State> {
  ...
  render() {
    const { isOpen, dashboard, panel, onDismiss, theme } = this.props;
    const { tab } = this.state;
    const styles = getStyles(theme);

    return (
      <Modal className={styles.modal} title={this.renderTitle()} isOpen={isOpen} onDismiss={onDismiss}>
        <div className={styles.modalContent}>
          <TabContent>
            {tab === 'link' && <ShareLink dashboard={dashboard} panel={panel} />}
            {tab === 'snapshot' && <ShareSnapshot dashboard={dashboard} panel={panel} onDismiss={onDismiss} />}
            {tab === 'export' && <span>Export Content</span>}
          </TabContent>
        </div>
      </Modal>
    );
