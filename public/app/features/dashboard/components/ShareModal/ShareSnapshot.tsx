...
export class ShareSnapshot extends PureComponent<Props, State> {
  ...
  createSnapshot = (external?: boolean) => () => {
    ...
    const dash = Object.create(dashboard) as DashboardModel;
    dash.snapshot = {
      timestamp: new Date(),
    };

    if (!external) {
      dash.snapshot.originalUrl = window.location.href;
    }
  ...
  saveSnapshot = async (dashboard: DashboardModel, external?: boolean) => {
    ...
    const dash = dashboard.getSaveModelClone();
  ...
  scrubDashboard = (dash: DashboardModel) => {
    ...
    // });
    // _.each(dashboard.annotations.list, annotation => {
